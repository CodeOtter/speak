...
function Speak(libraries, moods, statements, segments) {
    ...
    this.libraries = libraries || [
        ...
        new SpeechLibrary('emotives', 'emo', 'n-time self-referencing',
            ...
            function(statement, position, mood) {
                ...
                word = getRandomItem(this.list);
                if(word == 'am' && statement.substr(previous, 3) == 'we ') {
                    word = 'are';
                }
        ...
        new SpeechLibrary('questions', 'q', 'questions', 
            ['if', 'can', 'will', 'shall', 'might', 'should', 'could', 'would']),
    ...
    this.statements = statements || new Array(
        'if %seg, %c %seg',
        '%seg...? %seg',
    ...
    this.segments = segments || new Array(
        '%s %a %p? %o',
        '%s %emo %d',
        '%p? %o %ref? %d',
        '%p %o %ref %o',
 ...
 * @returns    String
 */
Speak.prototype.getStatement = function(mood, stupidity, statements, segments) {
    var mood = this.getMood(mood),
    ...
    result = mood.exceptions.find(result);

    questions = this.getLibrary('questions');
    if(result.split(' ')[0] in questions.list) {
 ...
 * @param    WordException    A WordException SpeakComponent
 */
function Mood(name, influencers, punctuations, emoticons, exceptions) {
    this.name = name || requirementFailed('Name is required');;
...
WordException.prototype.find = function(statement) {
    for(var i in this.dictionary) {
        if(this.dictionary.hasOwnProperty(i)) {
            var results = new RegExp(i).exec(statement);
            if(results !== null) {
